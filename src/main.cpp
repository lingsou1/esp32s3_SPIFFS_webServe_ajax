...
程序说明:此程序用于演示如何通过网页控制LED引脚以及将A0引脚读数实时显示于网页中。
        通过ajax来实现局部刷新是该程序最重要的部分.
...
void esp32s3_webServe_init(void){
  esp32s3_webServe.on("/setLED", handleLED);
  esp32s3_webServe.on("/read_CAP_SENSOR_Value",handle_CAP_SENSOR);
...
void handleLED(){
 String ledState = "OFF";
 String LED_State = esp32s3_webServe.arg("LEDstate"); //参考xhttp.open("GET", "setLED?LEDstate="+led, true);
 Serial.println(LED_State);
 
 if(LED_State == "1"){
  digitalWrite(LED_A,HIGH); //LED 点亮
  digitalWrite(LED_B,HIGH); //LED 点亮
  ledState = "ON"; //反馈参数
 } else {
  digitalWrite(LED_A,LOW); //LED 熄灭
  digitalWrite(LED_B,LOW); //LED 熄灭
  ledState = "OFF"; //反馈参数
 }
}

void handle_CAP_SENSOR() {
 int a = analogRead(CAP_SENSOR_PIN);
 String CAP_SENSOR_Value = String(a);
 
 esp32s3_webServe.send(200, "text/plain", CAP_SENSOR_Value); //发送模拟输入引脚到客户端ajax请求
}
